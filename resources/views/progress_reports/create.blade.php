        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="p-6 sm:p-8">
                    <form action="{{ route('projects.progress.store', $project) }}" method="POST" class="space-y-6">
                        @csrf

                        <div class="space-y-1">
                            <label class="block text-sm font-semibold text-slate-700">Persentase (%) <span class="text-rose-500">*</span></label>
                            <div class="relative mt-1 rounded-xl shadow-sm">
                                <input type="number" min="0" max="100" name="percentage" value="{{ old('percentage') }}" placeholder="0 - 100" class="block w-full rounded-xl border-slate-200 pr-12 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition-colors bg-slate-50/50 hover:bg-white focus:bg-white">
                        ...
                        <div class="space-y-1">
                            ...
                            <textarea name="description" rows="4" placeholder="Jelaskan kemajuan proyek..." class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition-colors bg-slate-50/50 hover:bg-white focus:bg-white">{{ old('description') }}</textarea>
                        </div>
